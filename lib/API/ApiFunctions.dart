...
activateCodes() async {
  ...
  var response = await http.post(
    Uri.parse("$ipAddress/sky-auth/status/activate"),
...
deactivateCodes() async {
  ...
  var response = await http.post(
    Uri.parse("$ipAddress/sky-auth/status/deactivate"),
    body: jsonEncode(body),
    headers: {"access_token": ACCESSTOKEN},
  );
...
addProgram(var token) async {
  ...
  var response = await http.post(
    Uri.parse("$ipAddress/sky-auth/authorization/program/link_program"),
    body: jsonEncode(body),
    headers: {"access_token": ACCESSTOKEN},
  );
...
confirmIdentifier(identifier, identifierType, text) async {
  ...
  if (isOnline) {
    ...
    var response = await http.post(
      Uri.parse("$ipAddress/sky-auth/identifier/confirm_identifier"),
      body: jsonEncode(body),
      headers: {"access_token": ACCESSTOKEN},
    );
...
getProgramsForIdentifier() async {
  ...
  if (isOnline) {
    ...
    try {
      var response = await http.post(
        Uri.parse("$ipAddress/sky-auth/programs/user/identifier/programs"),
        body: jsonEncode(body),
      );
...
deleteIdentifier(var identifierType, var identifier, context) async {
  ...
  if (isOnline) {
    ...
    try {
      var response = await http.post(
        Uri.parse("$ipAddress/sky-auth/identifier/delete"),
        body: jsonEncode(body),
      );
...
authenticate(var programName) async {
  ...
  if (isOnline) {
    ...
    var response = await http.post(
      Uri.parse(
          "$ipAddress/sky-auth/authorization/biometric/code_confirmation"),
      headers: {"access_token": ACCESSTOKEN},
      body: jsonEncode(body),
    );
    if (response.statusCode == 200) {
...
addIdentifierToDB(String identifier, String identifierType) async {
  ...
  if (isOnline) {
    ...
    var response = await http.post(
      Uri.parse("$ipAddress/sky-auth/identifier"),
      headers: {"access_token": ACCESSTOKEN},
      body: jsonEncode(body),
    );
...
Future<String> LoginToApp(var name, var pass, var context) async {
  ...
  if (isOnline) {
    ...
    // print("unsername : " + username);
    // print("password : " + hashedPasswordString);
    ...
    final response = await http.post(
      Uri.parse('$ipAddress/sky-auth/authorization/login?username=' + username),
...
Future<int> getIdentifierAndTypes() async {
  ...
  if (isOnline) {
    ...
    print("getIdentifierAndTypes access token : $ACCESSTOKEN");


    Map<String, String> headerValues = {"access_token": ACCESSTOKEN};
    ...
    var responseIdentifiers = await http.post(
        Uri.parse("$ipAddress/sky-auth/identifier/get"),
        headers: headerValues,
        body: jsonEncode(body));
    var responseIdentifierTypes = await http.get(
        Uri.parse('$ipAddress/sky-auth/identifier_type'),
      headers: headerValues);
    print(responseIdentifiers.statusCode);
    print(responseIdentifierTypes.statusCode);
...
Future<int> getPrograms() async {
  ...
  if (isOnline) {
    ...
    var response = await http.get(
      Uri.parse("$ipAddress/sky-auth/programs"),
      headers: {"access_token": ACCESSTOKEN},
    );
...
Future<int> getStatusCodes() async {
  ...
  if (isOnline) {
    var response = await http.get(
      Uri.parse("$ipAddress/sky-auth/auth_details?user_id=" +
          USERID.toString() +
          "&identifier=" +
          individualIdentifier),
      headers: {"access_token": ACCESSTOKEN},
...
Future<int> getAllStatusCodes() async {
  ...
  if (isOnline) {
    var response = await http.get(
      Uri.parse("$ipAddress/sky-auth/auth_details?user_id=$USERID"),
      headers: {"access_token": ACCESSTOKEN},
    );
    if (response.statusCode == 200) {
      try {
...
Future<String> signUp(String fName, String lName, String uName, String natID,
  ...
  if (isOnline) {
    try {
      ...
      print(userSignupDetails);

      final response = await http.post(Uri.parse('$ipAddress/sky-auth/users'),
...
deleteProgram(identifier, progid) async {
  ...
  if (isOnline) {
    ...
    try {
      var response = await http.post(
          Uri.parse("$ipAddress/sky-auth/programs/remove"),
          body: jsonEncode(body));
